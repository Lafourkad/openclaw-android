    ...
    private fun startMemoryPalace() {
        GlobalScope.launch(Dispatchers.IO) {
            try {
                ...
                server.start()
                memoryServer = server
                emitLog("[memory-palace] Ready on :18795")
                Log.i("OpenclawGW", "Memory Palace started on :18795")
            } catch (e: Exception) {
    ...
    private fun seedWorkspaceTools(filesDir: String) {
        try {
            ...
            val content = buildString {
                ...
                val palaceServerFile = File("$filesDir/.openclaw/extensions/memory-palace-android/openclaw.plugin.json")
                if (palaceServerFile.exists()) {
                    appendLine("## Memory Palace (on-device)")
                    appendLine("- **Backend:** SQLite (Room) + HNSW vector index")
                    appendLine("- **Embeddings:** Granite 278M (LiteRT/TFLite, 768 dims)")
                    appendLine("- **HTTP server:** http://127.0.0.1:18795")
                    appendLine("- **Plugin:** memory-palace-android (auto-loaded)")
                    appendLine("- **Tools:** memory_set, memory_recall, memory_get, memory_recent, memory_link, memory_unlink, memory_archive, memory_stats, memory_audit")
                    appendLine("- Fully on-device — no cloud, no PostgreSQL, no Ollama")
                    appendLine("- Embeddings generated by Granite model running on CPU")
                    appendLine("- HNSW index persisted at ~/.openclaw/memory/hnsw.index")
                    appendLine("- Use memory_set to store knowledge, memory_recall to search semantically")
                    appendLine()
                }
...
""")
    ...
    private fun patchOpenclawConfig(filesDir: String) {
        try {
            ...
            // Inject Memory Palace Android plugin — extract from APK assets to extensions dir
            val destPluginDir = File("$filesDir/.openclaw/extensions/memory-palace-android")
            try {
                destPluginDir.mkdirs()
                File(destPluginDir, "dist").mkdirs()
                val assetBase = "flutter_assets/assets/plugins/memory-palace-android"
                for (name in listOf("openclaw.plugin.json", "package.json", "dist/index.js")) {
                    applicationContext.assets.open("$assetBase/$name").use { input ->
                        File(destPluginDir, name).outputStream().use { output ->
                            input.copyTo(output)
                        }
                    }
                }
                Log.i("OpenclawGW", "patchConfig: Extracted palace plugin from APK assets to ${destPluginDir.absolutePath}")
            } catch (e: Exception) {
                Log.w("OpenclawGW", "patchConfig: Failed to extract palace plugin from assets: ${e.message}")
                // Fallback: try sdcard (for dev/ADB push)
                val extStorage = android.os.Environment.getExternalStorageDirectory()
                val srcPluginDir = File(extStorage, "openclaw-plugins/memory-palace-android")
                if (srcPluginDir.exists() && File(srcPluginDir, "openclaw.plugin.json").exists()) {
                    try {
                        destPluginDir.mkdirs()
                        File(destPluginDir, "dist").mkdirs()
                        for (name in listOf("openclaw.plugin.json", "package.json", "dist/index.js")) {
                            val src = File(srcPluginDir, name)
                            val dst = File(destPluginDir, name)
                            if (src.exists()) src.copyTo(dst, overwrite = true)
                        }
                        Log.i("OpenclawGW", "patchConfig: Copied palace plugin from sdcard fallback")
                    } catch (e2: Exception) {
                        Log.e("OpenclawGW", "patchConfig: Failed to copy palace plugin: ${e2.message}")
                    }
                }
            }
            // Configure plugin if it exists in extensions dir
            if (destPluginDir.exists() && File(destPluginDir, "openclaw.plugin.json").exists()) {
                val plugins = obj.optJSONObject("plugins") ?: org.json.JSONObject()
                // Plugin entry config
                val entries = plugins.optJSONObject("entries") ?: org.json.JSONObject()
                val palaceEntry = entries.optJSONObject("memory-palace-android") ?: org.json.JSONObject()
                palaceEntry.put("enabled", true)
                ...
                // Set memory slot
                val slots = plugins.optJSONObject("slots") ?: org.json.JSONObject()
                slots.put("memory", "memory-palace-android")
